...
/**
 * Plugin Name: SIDEUP KSA
 * Description: WooCommerce integration for SIDEUP - Saudi Arabia
 * Author: SIDEUP
 * Author URI: https://sa.sideup.co/
 * Version: 1.0
...
if (!function_exists('getBaseUrlSideupKSAApi')) {
    function getBaseUrlSideupKSAApi()
    {
        return 'https://portal.sa.sideup.co';
//    return 'http://sa.sideup.test';
    }
}
...
{
    $redirect_url = getBaseUrlSideupKSAApi().'/merchants/shipments';
    wp_redirect($redirect_url);
}
...
function sideup_wco_load()
{
    ...
    $url = getBaseUrlSideupKSAApi().'/api/getWooCommerceData/';
...
function sideup_wco_column_cb_data($colname, $orderId)
{
    ...
    $url = getBaseUrlSideupKSAApi().'/api/getWooCommerceData/' . $key;
...
//            echo "---";
    ...
    if($colname == 'shipping_request' && $sideup_status == 'to_be_picked') {
        ...
        $text = 'Cancel';
        echo "<a href='#'><span class='btn btn-danger cancel-button' data-key='". esc_attr($key) ."' bearerToken='". esc_attr($bearerToken) ."'>".esc_html($text)."</span></a>";
    }
    if(!$isTrashed && $colname == "shipping_request" && (empty($sideup_status) || $sideup_status == 'cancelled') && !in_array($sideup_status, $custom_status)) {
        ...
        echo "<a href='#'><span class='btn btn-success newShipping-button' data-key='". esc_attr($key) ."' bearerToken='". esc_attr($bearerToken) ."'
                data-city='". esc_attr($city) ."' data-total='". esc_attr($total) ."'>Ship</span></a>
                <input type='hidden' bearerToken='". esc_attr($bearerToken) ."' id='". esc_attr($key) ."' data-city='". esc_attr($city) ."' data-description='". esc_attr($desc) ."'
                data-total='". esc_attr($total) ."' data-address='". esc_attr($shipping_address) ."' data-mobile='". esc_attr($mobile) ."'
                data-name='". esc_attr($customer_first_name." ". $customer_last_name) ."' data-wordpress-id='". esc_attr($orderId) ."' data-backup-phone='". esc_attr("") ."'>
...
//            echo "---";
    ...
    if ($colname == 'payment_column') {
        ...
        if (!empty($sideup_payment)) {
            echo esc_html($sideup_payment) ." <br> <span class='badge badge-warning'>".esc_html($sideup_payment_status)."</span>";
...
function sideup_wpdocs_selectively_enqueue_admin_script( $hook ) {
    ...
    wp_enqueue_script( 'my_custom_script', plugin_dir_url( __FILE__ ) . 'js/sideup-1.0.js', array(), '1.0' );
}
...
add_filter('woocommerce_states_new', 'sideup_cities_and_zones');
